"""
Script for implementation of our simple example of a block cipher
based on a 3-round simplified Feistle network combined with AES boxes.

The implementation of this example cipher is given below.

Author: Dr. Marios O. Choudary
"""


import sys
import random
import string
import time
import itertools
import operator
import base64

from format_funcs import *

debug = False

# Rijndael S-box
sbox = []

# Rijndael Inverted S-box
rsbox = []


def icc_enc(k, m, getp=False):
    """ 
    Encrypt a message m with a key k in ECB mode using our example cipher as follows:
    c = icc_enc(k, m)

    Args:
      m should be a hexstring of 16 hex characters -- 8 bytes -- (m = L0 | R0)
      k should be a hexstring of 24 hex characters -- 12 bytes -- (k = k1 | k2 | k3)
      getp, if given and True, will return the result of the encryption
      at the intermediate point for a differential attack. 
      Note that in this case the output may not have 8 bytes.

    Return:
      The hexstring ciphertext c, with length 8 bytes (c = L3 | R3)
    """

    # Check legths
    if len(m) != 16:
        print("Wrong len for m")
        return ""

    if len(k) != 24:
        print("Worng len for k")
        return ""

    # Initialize L/R
    L = m[0:8]
    R = m[8:16]

    for r in range(3):
        if debug:
            print("Before round " + str(r+1) + "\n")
            print("L = " + L + ", R = " + R + "\n")

        if r == 2 and getp == True:
            return L

        # Get round key from k
        kr = k[8*r:8*(r+1)]

        LL = L  # Keep old value of left half

        # Update left half of output for this round
        L = R

        # Add round key
        tmp = hexxor(kr, R)

        # Go through S-box for each byte
        RR = ""
        for i in range(4):
            tmpi = int(tmp[2*i:2*(i+1)], 16)
            tmpis = sbox[tmpi]
            tmpish = int2hexstring(tmpis)
            RR = RR + tmpish

        R = hexxor(RR, LL)

    # Get final ciphertext
    c = L + R

    return c


def icc_dec(k, c, getp=False):
    """ 
    Decrypt a ciphertext c with a key k in ECB mode using our example cipher as follows:
    m = icc_dec(k, c)

    Args:
      c should be a hexstring of 16 hex characters -- 8 bytes -- (c = L0 | R0)
      k should be a hexstring of 24 hex characters -- 12 bytes -- (k = k1 | k2 | k3)
      getp, if given and True, will return the result of the encryption
      at the intermediate point for a differential attack.

    Return:
      The hexstring message m, with length 8 bytes (m = L0 | R0)
    """

    # Check legths
    if len(c) != 16:
        print("Wrong len for c")
        return ""

    if len(k) != 24:
        print("Worng len for k")
        return ""

    # Initialize L/R
    L = c[0:8]
    R = c[8:16]

    for r in range(2, -1, -1):
        if debug:
            print("Before round " + str(r+1) + "\n")
            print("L = " + L + ", R = " + R + "\n")

        if r == 1 and getp == True:
            return L

        # Get round key from k
        kr = k[8*r:8*(r+1)]

        RR = R  # Keep old value of right half

        # Update right half of output for this round
        R = L

        # Add round key
        tmp = hexxor(kr, R)

        # Go through S-box for each byte
        LL = ""
        for i in range(4):
            tmpi = int(tmp[2*i:2*(i+1)], 16)
            tmpis = sbox[tmpi]
            tmpish = int2hexstring(tmpis)
            LL = LL + tmpish

        L = hexxor(RR, LL)

    # Get plaintext
    m = L + R

    return m
